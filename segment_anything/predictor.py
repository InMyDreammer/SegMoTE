...
class SegMoTEPredictor:
    ...
    def set_image(self,image: np.ndarray, image_format: str = "RGB") -> None:
        ...
        self.features, self.interm_embeddings = self.model.image_forward(input_image.to(self.devices))
    ...
    def predict_torch(
        ...
        if boxes is not None:
            ...
            for i in range(boxes.shape[1]):
                prompt['bboxes'] = boxes[:, i, :]

                outputs, _, _ = self.model.forward_decoder(
                    self.features,
                    prompt,
                    interm_embeddings=self.interm_embeddings,
                    test_mode=self.model.test_mode,
                )
